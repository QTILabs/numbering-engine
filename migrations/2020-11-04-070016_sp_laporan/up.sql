...
-- Get Nomor Position
CREATE OR REPLACE FUNCTION sp_laporan_get_nomor_position(
	_id_jenis int, 
	_id_satker int, 
...
BEGIN
	RETURN (
		SELECT urutan 
			FROM laporan 
			WHERE 
				satker_id = _id_satker 
				AND jenis_id = _id_jenis 
				AND EXTRACT(MONTH FROM tanggal_laporan) = EXTRACT(MONTH FROM _tanggal_laporan) 
				AND EXTRACT(YEAR FROM tanggal_laporan) = EXTRACT(YEAR FROM _tanggal_laporan) 
				AND tanggal_laporan >= _tanggal_laporan 
				AND status = 'terkirim_sudah_diapprove' 
...
CREATE OR REPLACE FUNCTION sp_laporan_get_last_draft_nomor(
	_id_jenis int, 
	_id_satker int, 
	_tanggal_laporan timestampTz)
RETURNS INTEGER AS $$
...
-- Sort Nomor
CREATE OR REPLACE FUNCTION sp_laporan_sort_nomor(
	num_count int, 
	_id_jenis int, 
	_id_satker int, 
	_tanggal_laporan timestampTz)
...
DECLARE	
	...
	_urutan int;
BEGIN
	FOR data_laporan IN SELECT * FROM laporan 
		WHERE satker_id = _id_satker 
		AND jenis_id = _id_jenis 
		AND EXTRACT(MONTH FROM tanggal_laporan) = EXTRACT(MONTH FROM _tanggal_laporan) 
		AND EXTRACT(YEAR FROM tanggal_laporan) = EXTRACT(YEAR FROM _tanggal_laporan) 
		AND tanggal_laporan >= _tanggal_laporan 
		AND status = 'terkirim_sudah_diapprove' 
		ORDER BY tanggal_laporan ASC LOOP
			_urutan := num_count + 1;
			UPDATE laporan 
				SET nomor = sp_laporan_get_nomor(
					num_count,
					sp_laporan_get_kode_satker(_id_satker),
					sp_laporan_get_kode_referensi(_id_jenis),
					data_laporan.tanggal_laporan), 
				urutan = _urutan 
				WHERE id = data_laporan.id;
			num_count := _urutan;
	END LOOP;
END
$$ LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION sp_laporan_sort_old_nomor(
	num_count int, 
	_id_jenis int, 
	_id_satker int, 
	_tanggal_laporan timestampTz, 
...
CREATE OR REPLACE FUNCTION sp_laporan_re_sort_nomor(
	_id_jenis int, 
	_id_satker int, 
	_month int, 
	_year int)
...
BEGIN
	FOR data_laporan IN SELECT * FROM laporan 
		WHERE satker_id = _id_satker 
		AND jenis_id = _id_jenis 
		AND EXTRACT(MONTH FROM tanggal_laporan) = _month 
		AND EXTRACT(YEAR FROM tanggal_laporan) = _year 
		AND status = 'terkirim_sudah_diapprove' 
		ORDER BY tanggal_laporan ASC LOOP
			UPDATE laporan 
				SET nomor = sp_laporan_get_nomor(
