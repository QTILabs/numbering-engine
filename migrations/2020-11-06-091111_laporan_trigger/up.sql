...
INSERT ON public.laporan FOR EACH ROW
    WHEN (
        NEW.tanggal_laporan = CURRENT_TIMESTAMP 
        AND NEW.status = 'terkirim_sudah_diapprove'
        ) 
    EXECUTE PROCEDURE public.sp_laporan_insert();
...
_urutan int := COALESCE(
    sp_laporan_get_last_draft_nomor(NEW.jenis_id, NEW.satker_id, NEW.tanggal_laporan),
    sp_laporan_count_month_draft(NEW.jenis_id, NEW.satker_id,_month,_year)
);
...
INSERT ON public.laporan FOR EACH ROW
    WHEN (NEW.status = 'draft') 
        EXECUTE PROCEDURE public.sp_laporan_insert_draft();
...
INSERT ON public.laporan FOR EACH ROW
    ...
    EXECUTE PROCEDURE public.sp_laporan_insert_backdate();
...
_urutan int := COALESCE(
    ...
    sp_laporan_count_month_edit(NEW.jenis_id, NEW.satker_id,_month,_year,OLD.id) + 1
);
BEGIN 
...
UPDATE ON public.laporan FOR EACH ROW
    WHEN (
        OLD.tanggal_laporan IS DISTINCT FROM NEW.tanggal_laporan 
        OR OLD.jenis_id IS DISTINCT FROM NEW.jenis_id 
        OR OLD.satker_id IS DISTINCT FROM NEW.satker_id 
        AND NEW.status = 'terkirim_sudah_diapprove'
        ) 
    EXECUTE PROCEDURE public.sp_laporan_edit();
...
_urutan int := COALESCE(
    sp_laporan_get_nomor_position(NEW.jenis_id, NEW.satker_id, NEW.tanggal_laporan),
    sp_laporan_count_month(NEW.jenis_id, NEW.satker_id,_month,_year) + 1
);
BEGIN NEW.nomor := sp_laporan_get_nomor(
    _urutan - 1,
    sp_laporan_get_kode_satker(NEW.satker_id),
...
UPDATE ON public.laporan FOR EACH ROW
    WHEN (OLD.status = 'draft' AND NEW.status = 'terkirim_sudah_diapprove') 
        EXECUTE PROCEDURE public.sp_laporan_edit_draft_status();
...
UPDATE ON public.laporan FOR EACH ROW
    WHEN (NEW.status = 'draft') 
        EXECUTE PROCEDURE public.sp_laporan_edit_back_to_draft();
...
UPDATE ON public.laporan FOR EACH ROW
    WHEN (NEW.status = 'terhapus' AND OLD.status = 'terkirim_sudah_diapprove') 
        EXECUTE PROCEDURE public.sp_laporan_delete();
...
DELETE ON public.laporan FOR EACH ROW
    WHEN (OLD.status = 'terkirim_sudah_diapprove') 
